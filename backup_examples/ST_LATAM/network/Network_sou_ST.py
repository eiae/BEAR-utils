...
sou_data = pd.read_excel(xlspath, sheet_name="Import_sou", index_col=1, header=1) 
sou_data = sou_data.iloc[: , 1:]
sou_data_agg = sou_data.iloc[: , -1]
sou_data = sou_data.iloc[: , :-1]

# number of countries
N = len(sou_data.columns)
T = len(sou_data.index)
...
# log difference data
sou_log_data = np.log(sou_data)
sou_diff_data = sou_log_data.diff()
sou_diff_data = sou_diff_data.iloc[1:,:]
sou_log_data_agg = np.log(sou_data_agg)
sou_diff_data_agg = sou_log_data_agg.diff()
sou_diff_data_agg = sou_diff_data_agg.iloc[1:]

# plot all 
sou_data.plot(figsize=(18, 6),legend=False)
plt.show() # data looks stationary ?
sou_diff_data.plot(figsize=(18, 6),legend=False)
plt.show() 

# plot specific countries
sou_diff_data[ 'souCO' ].plot(figsize=(18, 6))
plt.show()
...
sou_data.describe(include='all') 

# plot the heatmap of the correlation matrix 
plt.figure(figsize=(15,12))
sns.heatmap(sou_data.corr(),xticklabels=sou_data.columns,yticklabels=sou_data.columns,cmap='terrain',linecolor='white', linewidths=1, annot=True) # the partial correlation Network is characterized by the inverse of the covariance matrix, so interesting to look at correlation matrix
plt.show() 
plt.figure(figsize=(15,12))
sns.heatmap(sou_diff_data.corr(),xticklabels=sou_diff_data.columns,yticklabels=sou_diff_data.columns,cmap='terrain',linecolor='white', linewidths=1, annot=True) 
plt.show() 
...
sou_data_idio = pd.DataFrame(columns=sou_data.columns,index=sou_data.index) # empty dataframe <- regressing each of the tickers to the factor and save the residuals here
sou_diff_data_idio = pd.DataFrame(columns=sou_diff_data.columns,index=sou_diff_data.index)

common_factor_sou = sou_data_agg #sou_data.mean(axis=1) # market factor as mean of all countries
common_factor_sou_diff = sou_diff_data_agg 

# change level vs differenced version
data = sou_data
factor = common_factor_sou
idio = sou_data_idio
...
# break sample into evenly spaced time windows to run rolling window of network
# t = round(T/3)
# idio_all = idio
# idio_1 = idio.iloc[0:t,:] 
# idio_2 = idio.iloc[t:t*2,:] 
# idio_3 = idio.iloc[t*2:t*3,:] 
...
net = glasso(alpha=10, max_iter=1000).fit( idio ) # first fixed tuning parameter

# also use the following line to tune automatically the shrinkage parameters
net = glasso_cv(max_iter=1000).fit( idio ) # choose optimal shrinkage
...
K = glasso(alpha=net.alpha_).fit( idio ).precision_
A = np.zeros( (N, N) )
for i in range(N):
    for j in range(i-1):
        if abs(K[i,j] > 0 ):
...
plt.scatter( (0.3,net.alpha_), (sparsity_example,sparsity_opt),s=500)
