...
pot_data = pd.read_excel(xlspath, sheet_name="Import_pot", index_col=1, header=1) 
pot_data = pot_data.iloc[: , 1:]
pot_data_agg = pot_data.iloc[: , -1]
pot_data = pot_data.iloc[: , :-1]

# number of countries
N = len(pot_data.columns)
T = len(pot_data.index)
...
# log difference data
pot_log_data = np.log(pot_data)
pot_diff_data = pot_log_data.diff()
pot_diff_data = pot_diff_data.iloc[1:,:]
pot_log_data_agg = np.log(pot_data_agg)
pot_diff_data_agg = pot_log_data_agg.diff()
pot_diff_data_agg = pot_diff_data_agg.iloc[1:]

# plot all 
pot_data.plot(figsize=(18, 6),legend=False)
plt.show() # data looks stationary ?
pot_diff_data.plot(figsize=(18, 6),legend=False)
plt.show() 

# plot specific countries
pot_diff_data[ 'potCO' ].plot(figsize=(18, 6))
plt.show()
...
pot_data.describe(include='all') 

# plot the heatmap of the correlation matrix 
plt.figure(figsize=(15,12))
sns.heatmap(pot_data.corr(),xticklabels=pot_data.columns,yticklabels=pot_data.columns,cmap='terrain',linecolor='white', linewidths=1, annot=True) # the partial correlation Network is characterized by the inverse of the covariance matrix, so interesting to look at correlation matrix
plt.show() 
plt.figure(figsize=(15,12))
sns.heatmap(pot_diff_data.corr(),xticklabels=pot_diff_data.columns,yticklabels=pot_diff_data.columns,cmap='terrain',linecolor='white', linewidths=1, annot=True) 
plt.show() 
...
pot_data_idio = pd.DataFrame(columns=pot_data.columns,index=pot_data.index) # empty dataframe <- regressing each of the tickers to the factor and save the residuals here
pot_diff_data_idio = pd.DataFrame(columns=pot_diff_data.columns,index=pot_diff_data.index)

common_factor_pot = pot_data_agg #pot_data.mean(axis=1) # market factor as mean of all countries
common_factor_pot_diff = pot_diff_data_agg 

# change level vs differenced version
data = pot_diff_data
factor = common_factor_pot_diff
idio = pot_diff_data_idio
