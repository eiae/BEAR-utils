...
for i in range(len(press)):
    ...
    for j in range(len(tension)):
        for h in range(len(finance)):
            ...
            xls["xls_"+country[0]+"_"+tension[j]+"_"+press[i]+"_"+finance[h]] = pd.ExcelFile("results_robustness_"+tension[j]+"_"+press[i]+"_"+"macro"+"_"+finance[h]+"_"+"monthly"+".xlsx")              
        ...
        for h in range(len(finance)):
            for row in range(v):
                for col in range(v):
                    ...
                    ax.plot(bar_l_0, output["output_"+res[0]]["output_"+res[0]+"_"+press[i]][res[0]+"_"+tension[j]+"_"+finance[h]+"_"+str(row+1)+"_"+str(col+1)]["median"], color=colours[i], linewidth=2, alpha=0.8, label="median")
                    ax.plot(bar_l_0, output["output_"+res[0]]["output_"+res[0]+"_"+press[i]][res[0]+"_"+tension[j]+"_"+finance[h]+"_"+str(row+1)+"_"+str(col+1)]["lw. bound"], color=colours[i], linewidth=1, linestyle=(0, (5, 1)), alpha=0.6, label="16% credible set")
                    ax.plot(bar_l_0, output["output_"+res[0]]["output_"+res[0]+"_"+press[i]][res[0]+"_"+tension[j]+"_"+finance[h]+"_"+str(row+1)+"_"+str(col+1)]["up. bound"], color=colours[i], linewidth=1, linestyle=(0, (5, 1)), alpha=0.6, label="84% credible set")
                    ax.fill_between(horizon, np.asarray(output["output_"+res[0]]["output_"+res[0]+"_"+press[i]][res[0]+"_"+tension[j]+"_"+finance[h]+"_"+str(row+1)+"_"+str(col+1)]["lw. bound"], dtype=float), np.asarray(output["output_"+res[0]]["output_"+res[0]+"_"+press[i]][res[0]+"_"+tension[j]+"_"+finance[h]+"_"+str(row+1)+"_"+str(col+1)]["up. bound"], dtype=float), color=colours[i], alpha=0.2)
                    ...
                    fig.savefig(os.path.join(OUTPUTS["OUTPUTS_"+country[0]+"_"+press[i]], res[0]+"_"+country[0]+"_"+press[i]+"_"+tension[j]+"_"+finance[h]+"_"+"monthly"+"_"+str(row+1)+"_"+str(col+1)+".png"), dpi=200, bbox_inches="tight")
